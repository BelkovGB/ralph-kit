  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'number',
        hint: 'Ревью отклонило работу — issue возвращается в очередь с замечаниями в теле. После этого числа отказов подряд Ralph откладывает её. При включённой Лизе он сразу просит её о помощи; иначе берёт следующую issue. Отложенная issue не даёт закрыть milestone.',
        unit: null,
        default: 3,
      },
      {
        ...
        section: 'Лиза',
        label: 'Вызывать Лизу после остановки',
        type: 'boolean',
        hint: 'Лиза разбирает незавершённую работу и может вернуть её Ralph. Ручная остановка и stopAfterFirstIssue её не вызывают.',
        unit: null,
        default: false,
      },
      {
        path: 'supervisor.agentCli',
        section: 'Лиза',
        label: 'CLI Лизы',
        type: 'select',
        options: agentClis,
        hint: 'Отдельный от Ralph агент: Codex или Claude Code.',
        unit: null,
        default: 'codex',
      },
      {
        ...
        optionsDependOn: 'supervisor.agentCli',
        allowCustom: true,
        hint: 'Список зависит от CLI Лизы. Новое имя модели можно ввести вручную.',
        unit: null,
        default: 'gpt-6-astra',
      },
      {
        path: 'supervisor.effort',
        section: 'Лиза',
        label: 'Effort Лизы',
        type: 'select',
        options: effortOptions,
        optionsDependOn: 'supervisor.agentCli',
        hint: 'Глубина рассуждения отдельной сессии Лизы.',
        unit: null,
        default: 'low',
      },
      {
        ...
        type: 'number',
        hint: 'Счётчик сохраняется между запусками и обнуляется в новой фазе.',
        unit: null,
        default: 3,
      },
      {
        path: 'supervisor.maxAdditionalIterations',
        section: 'Лиза',
        label: 'Дополнительных итераций',
        type: 'number',
        hint: 'Ограниченный резерв Ralph после помощи Лизы. Каждое успешное вмешательство добавляет не больше одной итерации.',
        unit: null,
        default: 3,
      },
      {
        path: 'supervisor.maxTurns',
        section: 'Лиза',
        label: 'Шагов в сессии Лизы',
        type: 'number',
        hint: 'Отдельный лимит шагов для одного вызова.',
        unit: null,
        default: 50,
      },
      {
        path: 'supervisor.timeoutMs',
        section: 'Лиза',
        label: 'Время сессии Лизы',
        type: 'number',
        hint: 'Без значения берётся runtime.agentTimeoutMs.',
        unit: 'мс',
        default: null,
      },
      {
        ...
        type: 'number',
        hint:
          'Считается только техническая ошибка: сессия упала, замолчала или не отдала файл результата. Общий таймаут и лимит шагов повтору не подлежат — на них Ralph останавливает прогон сразу. Отказ ревью по существу сюда тоже не входит. От 1 до 5: попытка — это целая сессия агента. Когда попытки кончились, коммит сохраняется, и следующий запуск повторит ревью без повторной реализации.',
        unit: null,
        default: 3,
      },
      {
      ...
      {
        path: 'runtime.agentFirstEventTimeoutMs',
        section: 'Таймауты',
        label: 'Ожидание первого события агента',
        type: 'number',
        hint: 'Сколько Ralph ждёт первое JSON-событие после запуска CLI. Если поток не начался, процесс завершается. Ревью запускается повторно как после временной ошибки. 300000 — пять минут.',
        unit: 'мс',
        default: 300000,
      },
      {
        path: 'runtime.agentIdleTimeoutMs',
        section: 'Таймауты',
        label: 'Допустимая тишина агента',
        type: 'number',
        hint: 'Сколько Ralph ждёт следующее JSON-событие работающего CLI. Каждое событие запускает отсчёт заново. После таймаута процесс завершается, а ревью запускается повторно. 600000 — десять минут.',
        unit: 'мс',
        default: 600000,
      },
    ],
