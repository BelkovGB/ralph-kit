...
export const fieldGroups = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'boolean',
        hint:
          'Ralph запоминает заголовок и тело issue в тот момент, когда берёт её в работу. Правка issue на GitHub после этого останавливает прогон. Выключите — и текст каждой issue придётся вписать в файл одобренных issues руками, а рядом обновить контрольную сумму этого файла в ralph-config.mjs.',
        unit: null,
        default: true,
      },
      {
      ...
      },
      {
        ...
        section: 'Откуда берутся issues',
        label: 'Файл одобренных issues',
        type: 'text',
        hint:
          'Отсюда Ralph читает issues, одобренные вручную. Сам он в этот файл не пишет: при автоодобрении запомненный текст лежит в состоянии прогона и уходит вместе с ним. Путь считается от корня проекта. Содержимое защищено контрольной суммой в ralph-config.mjs: правка файла без правки суммы останавливает прогон.',
        unit: null,
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'number',
        hint: 'Сколько Ralph ждёт сборку docker-образа проверок. На пределе сборка обрывается. 1800000 — 30 минут.',
        unit: 'мс',
        default: 1800000,
      },
      {
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'list',
        hint:
          'Готовят окружение: миграции, генерация кода, установка того, что не попало в образ. Ralph выполняет их в начале каждой фазы, а потом первыми перед каждым набором проверок, в том же контейнере. По одной команде оболочки на строку, до 500 символов, перенос строки внутри команды запрещён.',
        unit: null,
        default: [],
      },
      {
        ...
        type: 'list',
        hint: 'Доказывают, что изменение работает: линтер, сборка, тесты. Контейнер выполняет строки подряд и останавливается на первой ошибке. По одной команде оболочки на строку, до 500 символов, перенос строки внутри команды запрещён.',
        unit: null,
        default: [],
      },
      {
