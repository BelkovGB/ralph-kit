...
export const fieldGroups = [
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'number',
        hint: 'Ralph повторяет команду к GitHub или git, когда та упала по временной причине: таймаут, обрыв связи, ответ 5xx. Тем же числом повторяется сборка образа валидации — единственный шаг проверок с сетью. От 1 до 60: попытка стоит секунд ожидания, поэтому запас большой.',
        unit: null,
        default: 3,
      },
      {
      ...
      },
      {
        ...
        type: 'number',
        hint:
          'Сколько Ralph ждёт команду git, gh или docker. На пределе он убивает процесс и считает команду упавшей. Команды к сети — fetch, push, запросы к GitHub — после этого повторяются, остальные нет. 300000 — пять минут.',
        unit: 'мс',
        default: 300000,
      },
      {
        ...
        type: 'number',
        hint: 'Сколько Ralph ждёт сборку docker-образа проверок. На пределе сборка обрывается. Счёт идёт на каждую попытку: оборванная сборка повторяется столько раз, сколько задано полем «Повторов сетевой команды». 1800000 — 30 минут.',
        unit: 'мс',
        default: 1800000,
      },
      {
  ...
  },
  {
    ...
    fields: [
      {
        path: 'validationMode',
        label: 'Режим проверок',
        type: 'select',
        options: ['host', 'container'],
        hint:
          'host запускает команды прямо в проекте и проверяет, что они не изменили исходники. container собирает изолированный образ без сети. Контейнерный режим нужен для недоверенных репозиториев и воспроизводимой среды.',
        unit: null,
        default: 'container',
      },
      {
      ...
      },
      {
        ...
        type: 'text',
        hint: 'Путь от корня проекта к Dockerfile этого образа. Сети внутри прогона нет, поэтому всё нужное образ ставит при сборке.',
        unit: null,
        default: null,
      },
      {
        ...
        type: 'list',
        hint:
          'Абсолютные пути внутри контейнера, которым нужен временный том: корень образа смонтирован только для чтения, а инструменту нужно писать — например в свой кэш. Тома удаляются вместе с контейнером. Пути /source, /workspace и /tmp заданы Ralph и здесь не повторяются.',
        unit: null,
        default: [],
      },
      {
        ...
        type: 'list',
        hint:
          'Готовят окружение: миграции, генерация кода, установка зависимостей. Ralph выполняет их в начале каждой фазы, а потом первыми перед каждым набором проверок. В host-режиме цепочки запрещены: одна команда на строку. До 500 символов, перенос строки внутри команды запрещён.',
        unit: null,
        default: [],
      },
      {
        ...
        type: 'list',
        hint: 'Доказывают, что изменение работает: линтер, сборка, тесты. Ralph выполняет строки подряд и останавливается на первой ошибке. В host-режиме цепочки запрещены: одна команда на строку. До 500 символов, перенос строки внутри команды запрещён.',
        unit: null,
        default: [],
      },
      {
      ...
      },
      {
        ...
        type: 'list',
        hint: 'Манифесты и lock-файлы вашего стека, которые Ralph копирует в слой зависимостей образа. Пути относительные, внутри проекта. Правка такого файла пересобирает образ, а незакоммиченная правка останавливает прогон.',
        unit: null,
        default: [],
      },
    ],
...
// репозитория, а `prompt`, `phases` и `approvedIssueSnapshotsHash` обязательны:
// конфиг без любого из них отклоняется.
