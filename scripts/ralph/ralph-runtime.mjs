 ...
 *
 * В него идёт весь вывод агента и всех команд: без ротации файл за несколько
 * дней дорастает до десятков мегабайт. Открыть такой файл после падения нельзя,
...
function rotatePersistentLog(logPath, startedAt, uniqueSuffix = randomUUID().slice(0, 8)) {
  ...
  // миллисекунды получили бы одно имя, а renameSync молча затёр бы первый
  // архив. Суффикс делает имя уникальным по построению, а не по надежде, что
  // часы успели тикнуть.
  //
  // Одной уникальности мало. Retention ниже сортирует имена, и при совпавшей
  // метке порядок задавал бы случайный суффикс — то есть удалялись бы
  // произвольные архивы, а не самые старые. На диске Windows восемь ротаций в
  // миллисекунду не укладываются, а на быстрой файловой системе укладываются.
  // Порядковый номер делает имя не просто уникальным, а сортируемым по
  // построению.
 ...
 * даёт мегабайты одинаковых строк, из-за которых журнал прогона нечитаем.
 ...
 *
 * Набор проверок печатает десятки тысяч строк, и на их фоне в консоли не
