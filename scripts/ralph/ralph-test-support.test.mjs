...
test('без .git возвращается путь для создания, как в снимке контейнера', () => {
  // В workspace контейнера проверок `.git` не существует вовсе: снимок собирают
  // из git ls-files. mkdir создаст обычный каталог с этим именем.
