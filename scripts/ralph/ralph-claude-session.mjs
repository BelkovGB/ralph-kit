 ...
 * попадает в секцию памяти, и префикс двух соседних сессий расходится на ровном
 * месте: без флага вторая сессия заново создаёт кэш префикса и платит за него
 * как за первый, с флагом — читает уже созданный.
 */
 ...
 * командную строку на первом переводе строки: многострочная схема доходит до
 * CLI как «{», и ревью падает с «--json-schema is not valid JSON».
 *
 ...
 * exit 1, пустой stdout. Отказ наступает до старта сессии, то есть роняет
 * каждое ревью и каждое milestone-ревью. Ключ снимается здесь, а не в файле
...
function claudeTelemetry(event) {
  ...
  return {
    ...
    // чтения кэша, и складывает его сводка.
    uncachedInputTokens: numberOrNull(usage.input_tokens),
    outputTokens: numberOrNull(usage.output_tokens),
    cacheReadTokens: numberOrNull(usage.cache_read_input_tokens),
    cacheCreationTokens: numberOrNull(usage.cache_creation_input_tokens),
...
function readClaudeEvent(line) {
  ...
  // ждёт. Без этой ветки run.log молчит всё время отката без единой строки.
  // Codex-backend такой дыры не имеет: он выводит item.type === 'error'.
  ...
  // Состояние квоты приходит отдельным событием. Пока окно открыто, оно шум;
  // когда закрывается — это единственная строка, объясняющая, почему прогон
  // встал, и когда его можно продолжить.
  ...
  // обычный message.id, поэтому без отдельной ветки оно считалось бы шагом и
  // становилось lastAgentMessage — то есть транзиентный 5xx съедал бы шаг из
  // maxTurns и подменял последнюю реплику агента текстом ошибки. Модель не
  // работала, шага быть не должно; оператору сообщение всё равно нужно, но как
  // ошибка.
 ...
 * позже, а приходит этот отказ на самом дорогом шаге цикла. В справочнике
 * ошибок Anthropic он не описан вовсе; в трекере claude-code он тянется больше
 * года как перемежающийся сбой сервиса на активной подписке.
 *
 ...
 * тексте нет, поэтому проверка только по тексту промахивается — и цена
