  ...
  for (const line of stripAnsi(text).split(/\r?\n/)) {
    for (const pattern of failedTestPatterns) {
      ...
      const title = match[1]
        ...
        .replace(/\s+\(\d+(?:\.\d+)?ms\)$/, '')
        .trim();
      // `failing tests:` — заголовок сводки node:test, а не имя теста. В списке
      // упавших он даёт указание «перезапусти» на строке, которой не существует.
 ...
 * прошла валидацию. Без этого сессия начинается с нуля и заново выясняет, что
...
export function recoveryPrompt(storedIssue) {
  ...
  const focus =
    failedTests.length > 0
      ...
      : '';
  // Замечания переживают смену фазы: сессия исправления могла свалить проверки,
  // и тогда issue продолжается из `working-tree`, а ветка ревью выше уже не
  // срабатывает. Без этого блока агент чинит тесты и уходит на ревью с тем же
  // нарушением, круг повторяется до парковки issue.
  const pending = storedIssue?.reviewFindings?.trim()
