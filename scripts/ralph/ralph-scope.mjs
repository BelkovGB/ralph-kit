 ...
 * замечание, и цикл повторяет этот круг, пока не кончится бюджет итераций.
 */
 ...
 * каждое исправление трогает больше кода и даёт ему больше поверхности. Без
 * порога цикл ходит по одной задаче заход за заходом и не сходится, при том
 * что ни одно замечание не важнее P3.
 *
...
export function applySeverityFloor(review, floor, label = 'Review') {
  ...
  // Замечания уходят в отдельное поле, а не в никуда: вырезанные из `findings`,
  // они оставили бы отчёт с пустым списком под фразой «четыре замечания ниже
  // порога» — то есть ровно с тем, чего порог обещал не делать.
  return {
    ...review,
