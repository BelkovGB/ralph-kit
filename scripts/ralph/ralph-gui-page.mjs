...
export const commandGuide = [
  {
    ...
    items: [
      {
        title: 'Отчёт о готовности к прогону',
        command: 'node scripts/ralph/ralph-loop.mjs --check',
        does:
          'В консоли — состояние проекта: версия набора, фаза, репозиторий, milestone и число ' +
          'открытых issues, ветка, чистота дерева, остаток итераций, лимиты, модели и заголовок ' +
          'следующей issue. Если git, gh, CLI агента или конфиг не на месте, команда называет, ' +
          'чего именно не хватает.',
        when: 'Перед первым прогоном и после каждой правки .agents/ralph.config.json.',
        omits:
          'В проекте ничего не меняется: ветка та же, коммитов, issues и pull request не ' +
          'появляется. Вход в CLI агента не проверяется — это делает --run. Ветка, не совпавшая ' +
          'с веткой фазы, останавливает команду с ошибкой.',
      },
      {
        title: 'Тот же отчёт без ключа',
        command: 'node scripts/ralph/ralph-loop.mjs',
        does: 'То же, что --check: без аргумента это режим по умолчанию.',
        when: 'Отдельного повода нет: --check даёт тот же отчёт и явно называет режим.',
        omits: 'Прогон не начинается. Ralph работает только от явного --run.',
      },
      {
        title: 'Закрытые issues и pull request фазы',
        command: 'node scripts/ralph/ralph-loop.mjs --run',
        does:
          'На выходе — закрытые issues milestone и pull request фазы. Ralph берёт issues по ' +
          'возрастанию номера, отдаёт агенту, гоняет проверки в контейнере, коммитит, отправляет ' +
          'ветку и получает ревью.',
        when: 'Когда беклог на GitHub создан, а --check прошёл без ошибок.',
        omits:
          'Pull request остаётся не слитым: merge за вами. На одной фазе прогон не ' +
          'останавливается — закрыв milestone, берёт следующую фазу плана со своей веткой и своим ' +
  ...
  },
  {
    ...
    items: [
      {
        title: 'Документ требований по вашему описанию',
        command: '/prd описание фичи',
        does:
          'На выходе — файл docs/prd-слаг.md: пользовательские сценарии, допущения и критерии ' +
      ...
      },
      {
        ...
        omits: 'Найденное не чинится, issues не заводятся, прогон не останавливается.',
      },
    ],
  },
];
