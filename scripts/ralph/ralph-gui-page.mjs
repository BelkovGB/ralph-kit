...
export const commandGuide = [
    ...
    items: [
      {
        ...
        omits:
          "Не меняет файлы и не начинает выполнять задачу. В GitHub ходит только за чтением: репозиторий, права, milestone и очередь issues.",
      },
      {
        ...
        does:
          ...
          "отдаёт агенту, гоняет проверки, коммитит, отправляет " +
  ...
  },
  {
    ...
    note: "Перед работой Ralph-loop создайте в GitHub беклог: prd → plan-phase → issues. Навык solution-architect необязателен.",
    items: [
      {
      ...
      {
        title: "Архитектура — по необходимости",
        command: "/solution-architect docs/prd-слаг.md",
        does:
          "Создаёт docs/architecture-слаг.md: сравнивает варианты по PRD и существующему коду, " +
          "объясняет выбор, ограничения и последствия. При передаче плана проверяет его " +
          "соответствие архитектурным решениям.",
        when: "По запросу для сложной интеграции или изменения контрактов. Для обычного плана не требуется.",
        omits: "Код, задачи на GitHub и запуск Ralph в этот этап не входят.",
      },
      {
        ...
        does:
          ...
          "затронутые слои и критерии PRD. Подключённую архитектуру учитывает, если вы указали её. Фазу сдают целиком, " +
          "не дожидаясь остальных.",
        when: "После PRD, до создания задач. Архитектуру автоматически не создаёт.",
      ...
      },
      {
        ...
        does:
          ...
          "плана — Ralph берёт их по возрастанию номера. Источники закреплены за SHA коммита: " +
          "план и PRD, а при подключённой архитектуре — также её документ. Без архитектуры принимает и новые планы.",
        when: "Когда документы плана проверены, закоммичены и запушены.",
        omits:
      ...
      {
        title: "Приёмка фазы и регресс живым сценарием",
        command: "/acceptance Фаза 2. Корзина",
        does:
          "На выходе — кейсы фазы, дописанные в docs/acceptance/cases по модулям, отчёт прогона " +
          "с результатом по каждому кейсу и вердиктом «принято» или «не принято», и сводка " +
          "status.md. После прогона скилл называет модули, которые фаза задела, и спрашивает, " +
          "гнать ли по ним регресс. Регресс по команде: /acceptance regress cart или " +
          "/acceptance regress all; сводка без браузера: /acceptance status.",
        when: "Когда фаза закрыта, а pull request ещё черновик: перед тем как отдать работу людям.",
        omits:
          "Найденное не чинится, issues не заводятся, регресс без «да» не запускается. " +
          "Приложение скилл не поднимает и не гасит: адрес стенда и тестового пользователя берёт " +
          "из инструкций проекта, а не угадывает. Реестр модулей docs/acceptance/modules.md ведёт " +
          "проект; без него скилл предложит черновик и остановится.",
      },
    ],
  },
];
...
// Хвост допускает CRLF: копия у потребителя может приехать через канал с
// конверсией переводов строк, и оставшийся export убил бы весь скрипт молча.
const embeddedViewSource = viewSource.replace(/^export \{[^}]*\};\r?\n/mu, "");
    ...
    if (totals.metricsUnreadable) {
      frag.appendChild(
        el(
          'div',
          'empty',
          'Журнал issue-metrics.json лежит на месте, но не разбирается. Каталог журнала ' +
            'показывает git rev-parse --git-path ralph-loop. Расход не показать, пока файл ' +
            'не почините или не удалите.'
        )
      );
      return frag;
    }
    ...
    section.appendChild(
      el(
        ...
        'Пульт эти ключи не знает. Незнакомый ключ верхнего уровня и незнакомый ключ ' +
          ...
          'milestoneReview и фазы прогон пропускает молча и значение ' +
